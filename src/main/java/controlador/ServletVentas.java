	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("total") != null) {
			...
			if (i == 0) {
				...
				VentasDTO ven = new VentasDTO(cc, cu, iva, vv, tv);
				...
				if (ress) {
					...
					if (can > 0) {
						...
						dd = new DetalleVentaDTO(can, cv, codpro, total1+res,  res, total1 );
					...
					if (can > 0) {
						...
						dtv = ven.getTotalventa();
						dvv = ven.getValorventa();
						dd = new DetalleVentaDTO(can, cv, codpro2, total2+res1, res1, total2);
						dedao = new DetalleVentaDAO();
						x = dedao.Inserta_Cliente(dd);
						if (x) {
							//JOptionPane.showMessageDialog(null, "detalle insertado");
					...
					if (can > 0) {
						...
						dtv = ven.getTotalventa();
						dvv = ven.getValorventa();
						dd = new DetalleVentaDTO(can, cv, codpro3, total3+res2, res2, total3);
						dedao = new DetalleVentaDAO();
						x = dedao.Inserta_Cliente(dd);
						if (x) {
							...
							sesion.setAttribute("producto3", null);
